...
replace_pieces(Board, Color1, Color2, NewBoard):-
    length(Board, L),
    replace_pieces(Board, 1-1, L-L, Color1, Color2, NewBoard).

replace_pieces(Board, C-L, C-L, Color1, Color2, NewBoard):-
    get_piece(Board, C-L, Color1),
    set_piece(Board, C-L, Color2, NewBoard), !.
replace_pieces(Board, C-L, C-L, _ , _ ,Board).
  
replace_pieces(Board, C-L, C1-L1, Color1, Color2, NewBoard):-
    get_piece(Board, C-L, Color1),
    set_piece(Board, C-L, Color2, TempBoard),
    next_position(C-L, C1-L1, NextC-NextL),
    replace_pieces(TempBoard, NextC-NextL, C1-L1, Color1, Color2, NewBoard), !.
replace_pieces(Board, C-L, C1-L1, Color1, Color2, NewBoard):-
    next_position(C-L, C1-L1, NextC-NextL),
    replace_pieces(Board, NextC-NextL, C1-L1, Color1, Color2, NewBoard).
...
next_position(C-L, C-_, NextC-NextL):-
    NextL is L + 1,
    NextC is 1.
...
adjacent_stones(Board, C-L, Stones):-
    findall(Piece, 
            (   adjacent(C-L, C2-L2),                 % Get adjacent coordinates
                get_piece(Board, C2-L2, Piece)),   % Get the piece at the adjacent position
            Stones).
adjacent_black_stones_coordinates(Board, C-L, Stones):-
    findall(C2-L2, 
            (   adjacent(C-L, C2-L2),                 % Get adjacent coordinates
                get_piece(Board, C2-L2, black)),   % Get the piece at the adjacent position
            Stones).

%helper predicate that gets the Piece in a given place(C-L) of the board
get_piece(Board, C-L, Piece):-
    nth1(L, Board, Row),
    nth1(C, Row, Piece).

%helper predicate that finds all the pieces of a given player in the board
find_player_pieces(Board, Player, Coordinates) :-
    findall(C-L, (
        nth1(L, Board, Row),
        nth1(C, Row, Player)
    ), Coordinates).
